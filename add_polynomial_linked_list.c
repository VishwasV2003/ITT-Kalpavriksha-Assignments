...
Node* add_polynomial_linked_list(Node* operand_1,Node* operand_2){
    Node* result=NULL;
    while(operand_1 && operand_2){
        if(operand_1->exponent==operand_2->exponent){
            int coffiecient=operand_1->coffiecient+operand_2->coffiecient;
            result=insert_node(result,make_node(coffiecient,operand_1->exponent));
            operand_1=operand_1->next;
            operand_2=operand_2->next;
        }
        else if(operand_1->exponent>operand_2->exponent){
            result=insert_node(result,operand_1);
            Node* temp=operand_1;
            operand_1=operand_1->next;
            temp->next=NULL;
        }
        else{
            result=insert_node(result,operand_2);
            Node* temp=operand_2;
            operand_2=operand_2->next;
            temp->next=NULL;
        }
...
void main(){
    ...
    printf("\n\n");
    Node* result=add_polynomial_linked_list(operand_1,operand_2);
